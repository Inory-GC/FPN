...
def parse_args():
    ...
    parser.add_argument('--load_dir', dest='load_dir',
                        help='directory to load models', default="/home/ai/FPN_Pytorch-master/weights",
                        type=str)
...
if __name__ == '__main__':
 ...
 #       args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]']
        args.set_cfgs = ['ANCHOR_SCALES', '[4, 8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]']  #修改
...
##--------------------------修改
    new_boxes = [[[] for _ in xrange(num_images)]  # [[[]], [[]]]
                 for _ in xrange(imdb.num_classes)]
    ...
    for i in range(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                # num_cls = 0
                # boxes_num = 0
                # for i in range(np.minimum(800, cls_dets.shape[0])):
                ...
                if vis:
                 ...
                 #   im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), 0.3)
                    im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), 0.7) #修改
                new_numpy = cls_dets.cpu().numpy().tolist()
             #   print(type(new_numpy))
                #print(new_numpy)
                det = []
                for box in new_numpy:
                    #print(box)
                    if box[4] > 0.7:
                        det.append(box)

                #all_boxes[j][i] = cls_dets.cpu().numpy()
                all_boxes[j][i] = np.array(det, dtype=np.float32)
                ...
                false_box = 0
                num = 0
                boxes_1 = all_boxes[1][0]
                print(boxes_1)
                for boxs_num in boxes_1:
                    scores_box = boxs_num[4]
                    if scores_box < 0.7:
                        false_box += 1
                    num += 1

                print('num_box:', num)
                print('false_box:', false_box)
                """
            else:
    ...
    # for image_num in all_boxes[1]:
    #     a = []
    #     threshould = 0.7
    #     for box_num in image_num:
    #         #print(box_num)
    #         score = box_num[4]
    #         if score > threshould:
    #             a.append(box_num.tolist())
    #     #print(np.array(a, dtype=np.float32))
    #
    # new_boxes[1].append(np.array(a, dtype=np.float32))
    # print("hhhhhhhhhhhhhhhhhhhhhhhhhhh", new_boxes)
    #
